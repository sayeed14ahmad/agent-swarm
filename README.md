...
* **Business Research Agent**
  ...
  * **Input:** Raw case constraints loaded dynamically from `baseline_case.txt` (or `surprise_case.txt`), capital limits, and segment demand parameters.
...
```bash
export AZURE_API_KEY="your_actual_api_key"
export AZURE_ENDPOINT="[https://your-resource-name.openai.azure.com](https://your-resource-name.openai.azure.com)"

```

**Execution:**
Ensure that `baseline_case.txt` and `surprise_case.txt` are present in the project root directory. You can run the engine via the command line or launch the interactive judges' UI:

```bash
# 1. Run the baseline scenario via CLI (outputs to Baseline_Trace.txt)
python boardroom_swarm.py baseline

# 2. Run the surprise round scenario via CLI (outputs to Surprise_Trace.txt)
python boardroom_swarm.py surprise

# 3. Launch the interactive Streamlit dashboard for judges
...
* **Framework:** Custom plain-Python sequential execution framework combined with a Streamlit UI dashboard (`app.py`). We intentionally excluded heavy orchestration frameworks to maintain absolute control over the STAGE logging mechanism and prevent API versioning conflicts with the Azure v1 endpoint.
* **Libraries:** Standard `openai` Python SDK (v1.0+) and `streamlit`.
...
* **Rigid Sequencing:** The 5-stage protocol is hardcoded to fire sequentially (Research → Finance/Marketing → Challenge → Compare → Decide) to guarantee compliance. It does not support dynamic, infinitely looping debate.
...
The core multi-agent execution loop, stage logging mechanism, dynamic text-file input reading, Streamlit dashboard integration, and prompt structures were custom-built entirely during this hackathon to meet the explicit guidelines of the Agentic Swarm Official Rulebook. The implementation utilizes standard Python libraries and the official OpenAI SDK for API connectivity. No pre-existing proprietary templates, external agentic libraries (CrewAI/AutoGen), or unauthorized third-party boilerplates were used in the creation of the swarm logic.
